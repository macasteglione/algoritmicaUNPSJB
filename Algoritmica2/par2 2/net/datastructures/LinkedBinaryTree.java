...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
	...
	public static boolean validExpressionTree(LinkedBinaryTree<String> t, List<String> op) {
		if (t.isEmpty())
			return false;
		if (!op.contains(t.root().getElement()))
			return false;
		return validExpressionTreeRecursive(t, t.root(), op);
	}

	private static boolean validExpressionTreeRecursive(LinkedBinaryTree<String> t, Position<String> current,
			List<String> op) {
		if (t.isExternal(current))
			return !op.contains(current.getElement());
		if (!op.contains(current.getElement()))
			return false;
		boolean leftValid = validExpressionTreeRecursive(t, t.left(current), op);
		boolean rightValid = validExpressionTreeRecursive(t, t.right(current), op);
